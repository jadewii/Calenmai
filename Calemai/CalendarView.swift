...
struct CalendarView: View {
    ...
    var body: some View {
            ...
            ZStack {
                ...
                VStack(spacing: 0) {
                    ...
                    CalendarGridView(
                        ...
                        availableHeight: geometry.size.height - 50 // Almost all space
                    )
        ...
        .onChange(of: taskStore.longPressedDate) { newDate in
            if newDate != nil {
...
struct CalendarGridView: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            VStack(spacing: 0) {
                // Day headers - compact but readable
                HStack(spacing: 0) {
                ...
                // Calendar dates with thin grid lines
                let dates = generateDates()
                let taskMap = tasksPerDay
                let rowCount = dates.count / 7
                let gridSpacing: CGFloat = 2 // Thin spacing for grid lines
                
                // Calculate cell dimensions accounting for spacing
                let headerHeight: CGFloat = 20
                let availableGridHeight = availableHeight - headerHeight
                let totalVerticalSpacing = gridSpacing * CGFloat(rowCount - 1)
                let cellHeight = (availableGridHeight - totalVerticalSpacing) / CGFloat(rowCount)
                let totalHorizontalSpacing = gridSpacing * 6 // 6 gaps between 7 columns
                let cellWidth = (geometry.size.width - totalHorizontalSpacing) / 7
...
struct CalendarDayView: View {
    ...
    var body: some View {
        ZStack {
            ...
            // Text on top - scale with cell height
            Text(dayText)
                .font(.system(size: min(24, cellHeight * 0.4), weight: isToday ? .black : .bold))
