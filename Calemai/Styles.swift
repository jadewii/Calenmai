...
// MARK: - Mode Switcher (EXACT from watchOS)
...
struct ModeSwitcherButton: View {
    ...
    var body: some View {
        ZStack {
            Circle()
                .fill(currentMode.modeButtonColor)
                .frame(width: TodomaiLayout.modeSwitcherSize, height: TodomaiLayout.modeSwitcherSize)
            
            Circle()
                .fill(Color.white)
                .frame(width: TodomaiLayout.modeSwitcherInnerSize, height: TodomaiLayout.modeSwitcherInnerSize)
        }
        .overlay(
            Circle()
                .stroke(Color.black, lineWidth: TodomaiLayout.modeSwitcherBorder)
        )
    }
}